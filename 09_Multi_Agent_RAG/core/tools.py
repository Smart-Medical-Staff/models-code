...
def tool_analyze_ppg_signal(state: Any, args: dict) -> dict:
    """
    Optional PPG neuropathy screening from uploaded red/IR waveform samples.
    Args: {"red": [...], "ir": [...], "sampling_rate": 100}
    """
    payload = dict(args or {})
    if not payload:
        payload = getattr(state, "ppg_payload", {}) or {}
    if not payload:
        return {"status": "skip", "message": "No PPG payload provided."}
    try:
        return DiagnosticService.run_ppg_assessment(payload)
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
TOOL_REGISTRY: dict[str, callable] = {
    ...
    "analyze_ppg_signal": tool_analyze_ppg_signal,
}
...
8. analyze_ppg_signal(red, ir, sampling_rate)
   → Optional experimental PPG screening using uploaded red/IR waveform samples.
   → Use only when a PPG payload is explicitly provided; never invent PPG data.
"""
