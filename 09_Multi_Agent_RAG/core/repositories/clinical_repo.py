...
class ClinicalRepository(BaseRepository):
    ...
    def save_nss_assessment(patient_id: str, total_score: int) -> None:
        val = int(round(float(total_score or 0)))
        ClinicalRepository.insert("nss_assessments", {"patient_id": patient_id, "total_score": val})

    @staticmethod
    def save_nds_assessment(patient_id: str, total_score: int) -> None:
        val = int(round(float(total_score or 0)))
        ClinicalRepository.insert("nds_assessments", {"patient_id": patient_id, "total_score": val})

    @staticmethod
    def save_gum_assessment(patient_id: str, total_score: int) -> None:
        val = int(round(float(total_score or 0)))
        ClinicalRepository.insert("gum_assessments", {"patient_id": patient_id, "total_score": val})

    @staticmethod
    def save_ulcer_assessment(patient_id: str, ulcer_stage: int) -> None:
        val = int(round(float(ulcer_stage or 0)))
        ClinicalRepository.insert("ulcer_assessments", {"patient_id": patient_id, "ulcer_stage": val})
