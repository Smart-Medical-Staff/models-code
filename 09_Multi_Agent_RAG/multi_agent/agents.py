...
class PlannerAgent:
    ...
    def run(self, state: MultiAgentState) -> MultiAgentState:
        ...
        selected = state.selected_assessments or []
        selected_str = ", ".join(selected) if selected else "to be chosen by patient"
...
Selected assessments: {selected_str}
Existing data: ML_stored={has_ml}, RAG_loaded={has_rag}

Generate a concise 4-6 step diagnostic plan for these assessments.
Respond ONLY as a JSON array of strings.
Example: ["Load patient history from RAG memory", "Run selected assessment modules", ...]"""
...
{{
  ...
  "confidence_valid": true/false
    ...
    def run(self, state: MultiAgentState) -> MultiAgentState:
        ...
        # Decide next node — go to report if we have any completed assessment results
        if (state.secondary_assessments_complete
                or state.has_fusion()
                or state.completed_assessments):
            state.next_node = NODE_REPORT
...
class ReportGeneratorAgent:
    ...
    def _section_gestational_overview(cls, gender: str, gd: dict) -> str:
        if gender == "Male":
            return "Gestational: Not applicable"
        if gd.get("skipped"):
            return "Gestational: Not applicable"
    ...
    @classmethod
    def _section_osteo_overview(cls, osteo: dict) -> str:
        ml_ost = osteo.get("ml_result") or {}
        if not ml_ost and not osteo.get("score"):
            return "Insufficient data for this section"
        parts = []
        if osteo.get("score") is not None:
            parts.append(f"Screening score: {osteo.get('score')}")
        if ml_ost:
            parts.append(f"Risk level: {cls._safe_get(ml_ost, 'risk_level', 'N/A')}")
            parts.append(f"Estimated probability: {cls._format_risk(ml_ost.get('predicted_probability'))}")
        return "\n".join(parts) if parts else "Insufficient data for this section"

    @classmethod
    def _section_shoulder_overview(cls, shoulder: dict) -> str:
        ml_shd = shoulder.get("ml_result") or {}
        if not ml_shd and not shoulder.get("score"):
            return "Insufficient data for this section"
        parts = []
        if shoulder.get("score") is not None:
            parts.append(f"Screening score: {shoulder.get('score')}")
        if ml_shd:
            parts.append(f"Risk level: {cls._safe_get(ml_shd, 'risk_level', 'N/A')}")
            parts.append(f"Estimated probability: {cls._format_risk(ml_shd.get('predicted_probability'))}")
        return "\n".join(parts) if parts else "Insufficient data for this section"

    @classmethod
    def _section_osteo_report(cls, osteo: dict) -> str:
        ml_ost = osteo.get("ml_result") or {}
        score = osteo.get("score")
        if not ml_ost and score is None:
            return "Insufficient data for this section"
        lines = ["Osteoporosis risk screening (secondary assessment)."]
        if score is not None:
            lines.append(f"Combined screening score: {score} (maximum reference 135).")
        if ml_ost:
            lines.append(f"Risk classification: {cls._safe_get(ml_ost, 'risk_level', 'N/A')}.")
            prob = ml_ost.get("predicted_probability")
            if prob is not None:
                lines.append(f"Estimated probability: {prob * 100:.1f}% ({ml_ost.get('method', 'N/A')}).")
        lines.append(
            "This result supports clinical risk assessment; confirm with a DEXA scan."
        )
        return "\n".join(lines)

    @classmethod
    def _section_shoulder_report(cls, shoulder: dict) -> str:
        ml_shd = shoulder.get("ml_result") or {}
        score = shoulder.get("score")
        if not ml_shd and score is None:
            return "Insufficient data for this section"
        lines = ["Frozen Shoulder risk screening (secondary assessment)."]
        if score is not None:
            lines.append(f"Combined screening score: {score} (maximum reference 148).")
        if ml_shd:
            lines.append(f"Risk classification: {cls._safe_get(ml_shd, 'risk_level', 'N/A')}.")
            prob = ml_shd.get("predicted_probability")
            if prob is not None:
                lines.append(f"Estimated probability: {prob * 100:.1f}% ({ml_shd.get('method', 'N/A')}).")
        lines.append(
            "This result supports joint evaluation; confirm with clinical range of motion testing."
        )
        return "\n".join(lines)
    ...
    def generate_report(cls, state: MultiAgentState) -> str:
        ...
        ppg = state.ppg_results or {}
        gd = state.gestational_results or {}
        hr = state.heart_risk_results or {}
        osteo = state.osteo_results or {}
        shoulder = state.shoulder_results or {}
        ...
        osteo_overview = cls._section_osteo_overview(osteo)
        shoulder_overview = cls._section_shoulder_overview(shoulder)
        ppg_overview = "Not provided"
        if ppg:
            if ppg.get("status") == "success":
                assessment_id = ppg.get("assessment_id") or state.ppg_assessment_id
                ppg_overview = (
                    f"Risk level: {cls._safe_get(ppg, 'risk_level', 'N/A')}\n"
                    f"Estimated probability: {cls._format_risk(ppg.get('neuropathy_probability'))}\n"
                    f"Signal quality: {cls._safe_get(ppg, 'signal_quality', 'N/A')}\n"
                    f"Stored assessment ID: {assessment_id or 'Not persisted'}\n"
                    "Experimental PPG screening only; confirm clinically."
                )
            else:
                ppg_overview = f"Unavailable: {cls._safe_get(ppg, 'message', 'PPG analysis did not complete')}"
        ...
        if ppg and ppg.get("status") == "success":
            risk_lines.append(
                f"Optional PPG screen: {ppg.get('risk_level')} "
                f"({ppg.get('neuropathy_probability', 0) * 100:.1f}%, "
                f"quality {ppg.get('signal_quality', 'N/A')})."
            )
        risk_summary = "\n".join(risk_lines)
        ...
        secondary_lines = [
            ...
            heart_overview,
            "",
            "5.3 Osteoporosis Risk Assessment",
            osteo_overview,
            "",
            "5.4 Frozen Shoulder Risk Assessment",
            shoulder_overview,
        ]
        ...
        if "Insufficient" not in osteo_overview:
            insights.append("Osteoporosis screening completed as a secondary check.")
        if "Insufficient" not in shoulder_overview:
            insights.append("Frozen Shoulder screening completed as a secondary check.")
        if ppg and ppg.get("status") == "success":
            insights.append("Optional PPG physiological screening was added as experimental context.")
        if not insights:
        ...
        if "Insufficient" not in osteo_overview:
            recommendations.append(
                "Discuss osteoporosis risk factors, calcium/Vitamin D intake, or bone density testing (DEXA) with your clinician."
            )
        if "Insufficient" not in shoulder_overview:
            recommendations.append(
                "Discuss shoulder range of motion restrictions and options like physical therapy with your clinician."
            )
        if ppg and ppg.get("status") == "success":
            recommendations.append(
                "Treat PPG output as a screening signal only and confirm neuropathy concerns with clinical testing."
            )
        ...
        if "Insufficient" not in osteo_overview:
            stratification.append(f"Osteoporosis risk (secondary): {osteo_overview.split(chr(10))[0]}.")
        else:
            stratification.append("Osteoporosis risk (secondary): Insufficient data for this section.")
        if "Insufficient" not in shoulder_overview:
            stratification.append(f"Frozen Shoulder risk (secondary): {shoulder_overview.split(chr(10))[0]}.")
        else:
            stratification.append("Frozen Shoulder risk (secondary): Insufficient data for this section.")
        if ppg:
            stratification.append(f"PPG screening (optional): {ppg_overview.split(chr(10))[0]}.")
        ...
        if ppg and ppg.get("status") == "success":
            neuropathy_interp_lines.append(
                "PPG physiological context: "
                f"{ppg.get('risk_level')} ({ppg.get('neuropathy_probability', 0) * 100:.1f}%)."
            )
        ...
        # Build sections — only include assessed modules
        completed = set(state.completed_assessments or [])
        neuropathy_done = "neuropathy" in completed
        gum_done    = "gum" in completed
        ulcer_done  = "ulcer" in completed
        heart_done  = "heart" in completed
        gd_done     = "gestational" in completed
        osteo_done  = "osteo" in completed
        shoulder_done = "shoulder" in completed

        sections = [
            "## Patient Diagnostic Report",
            "",
            cls._format_section("1. Patient Summary", patient_summary),
        ]

        # Neuropathy primary (only if assessed)
        if neuropathy_done or fusion_score is not None:
            sections.append(cls._format_section(
                "2. Primary Diagnosis (Neuropathy - PDN)",
                (
                    f"Primary diagnosis: {primary_decision}\n"
        ...
        # Risk summary (only relevant if any neuropathy/gum/ulcer done)
        if neuropathy_done or gum_done or ulcer_done:
            sections.append(cls._format_section("4. Risk Factors Summary", risk_summary))

        if ppg:
            sections.append(cls._format_section("4.1 Optional PPG Screening", ppg_overview))

        # Secondary overview — only list what was actually done
        secondary_lines = ["Completed assessments are listed below."]
        if gd_done or heart_done or osteo_done or shoulder_done:
            secondary_lines.append("")
            if gd_done:
                secondary_lines += ["Gestational Diabetes", gestational_overview, ""]
            if heart_done:
                secondary_lines += ["Cardiovascular Risk", heart_overview, ""]
            if osteo_done:
                secondary_lines += ["Osteoporosis Risk", osteo_overview, ""]
            if shoulder_done:
                secondary_lines += ["Frozen Shoulder Risk", shoulder_overview, ""]
            sections.append(cls._format_section(
                "5. Assessment Overview", "\n".join(secondary_lines)
            ))

        sections += [
            cls._format_section("6. Clinical Insights", clinical_insights),
            cls._format_section("7. Recommendations", "\n".join(f"- {r}" for r in recommendations)),
            cls._format_section("8. Risk Stratification Summary", "\n".join(stratification)),
            cls._format_section("9. Follow-up Advice", "\n".join(follow_up)),
        ]

        # Detailed reports — only for completed modules
        ...
        if gd_done:
            sections.append(cls._format_section(
                f"{sec_num}. Gestational Diabetes Report",
                cls._section_gestational_report(gender, gd)
            ))
            sec_num += 1
        if heart_done:
            sections.append(cls._format_section(
                f"{sec_num}. Cardiovascular Risk Report",
                cls._section_heart_report(hr)
            ))
            sec_num += 1
        if osteo_done:
            sections.append(cls._format_section(
                f"{sec_num}. Osteoporosis Risk Report",
                cls._section_osteo_report(osteo)
            ))
            sec_num += 1
        if shoulder_done:
            sections.append(cls._format_section(
                f"{sec_num}. Frozen Shoulder Risk Report",
                cls._section_shoulder_report(shoulder)
            ))
            sec_num += 1
